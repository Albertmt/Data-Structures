...
class KDTree
{
	 ...
	 **/
	double distSquared(Point<Dim> a, Point<Dim> b) const{
		double d = 0;
		for(int i = 0; i < Dim; i++)
			d += (a[i] - b[i]) * (a[i] - b[i]);
		return d;
	...
	/**
	 * Modifies the part of "points" vector from
	 * start to end to "look" like a KDTree.
	 **/
	void build(int start, int end, int dim);
	
	/**
	 * "points" vectori mej jarum a place 
	 * teghi element@ u jisht tegh@ dnum a.
	 **/
	void quickSelect(int start, int end, int place, int dim); 
	
	/**
	 * "pIndex" teghi element@ tanum, jisht teghn a dnum,
	 * iranic dzax sagh poqr en linum, iranic aj` mec (@st
	 * hamapatasxan dim-i). Veradardznum a verjnakan dirq@.
	 **/
	int partition(int start, int end, int pIndex, int dim){
		Point<Dim> pivotValue = points[pIndex];
	...
	/**
	 * Helper for findNearestNeighbor(). Finds and returns the
	 * closest Point to the Point query between "start" and "end"
	 * Points of the vector "points".
	 **/	
//	Point<Dim> fNNhelper(int start, int end, const Point<Dim> & query, int dim) const;
	void fNNhelper(int left, int right, int dimension, const Point<Dim> & query, Point<Dim> & currentBest, int minDis, bool & isFirst) const;
};
