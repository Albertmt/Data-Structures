...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const{
	...
	if(subroot -> elements.size() > idx and subroot -> elements[idx] == key	)
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    * Mid element is at index (3 - 1) / 2 = 1 .
    ...
    /* TODO Your code goes here! */
	DataPair middlePair = *mid_elem_itr;
	parent->elements.insert(elem_itr, middlePair);
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if(first_larger_idx < subroot->elements.size() and subroot->elements[first_larger_idx].key == pair.key)
			return;
