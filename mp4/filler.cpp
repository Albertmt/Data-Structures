...
struct Pixel{
	...
	int x; //column number of the pixel, from the left
	int y; //row number of the pixel, from the top
};

int RGBDistance(RGBAPixel first, RGBAPixel second){
	return (int)(first.red - second.red) * (first.red - second.red) + 
...
animation filler::fill( PNG & img, int x, int y,
	...
	animation a;
	Pixel P;
	RGBAPixel center = *img(x,y);
	int Nth = 0;
	bool **processed = new bool*[(int)img.width()];
	for (size_t i = 0; i < img.width(); i++){
		processed[i] = new bool[(int)img.height()];
		for(size_t j = 0; j < img.height(); j++)
			processed[i][j] = false;
	}
	OrderingStructure<Pixel> container;
	container.add(Pixel(x,y));
	while(!container.isEmpty()){
		P = container.remove(); // last element in the stack
		if(!processed[P.x][P.y] && RGBDistance(*img(P.x,P.y), center) <= tolerance){
			Nth++;
			processed[P.x][P.y] = true;
			*img(P.x,P.y) = fillColor(P.x, P.y);
			if(P.x != (int)img.width()-1) container.add(Pixel(P.x + 1, P.y));//right
			if(P.y != (int)img.height()-1) container.add(Pixel(P.x, P.y + 1));//down
			if(P.x != 0) container.add(Pixel(P.x - 1, P.y));//left
			if(P.y != 0) container.add(Pixel(P.x, P.y - 1));//up
			if(Nth % frameFreq == 0)
				a.addFrame(img);
		}
	}
